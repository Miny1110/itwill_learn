...
--컬럼을 많이 생성하고 동시에 PK를 선언하기 때문에, 많은 컬럼들 중에 어떤 컬럼이 PK가 될 것인지 적어주어야 한다.
--여기서는 NO 컬럼이 PK가 된다.
...
--FOREIGN KEY
--이미 만들어져 있는 테이블의 제약조건을 변경하는 것이기 때문에 ALTER를 사용한다.
...
--40 데이터는 들어가지 않는다.
--CUSTOMER 테이블이 참조하고 있는 ORDERS 테이블에는 40이라는 값이 없기 때문이다.

COMMIT;
--커밋을 하면 INSERT문이 DB에 들어간다. 커밋하기 전까지는 그냥 메모리에 있는 것이다.
...
INSERT INTO CATALOGS VALUES (2,'ABC',40);
...
--제약조건 범위에는 부등호를 사용해도 된다.

INSERT INTO ORDERS VALUES (70,601,'GGG',7); --X (CHECK 제약조건에 걸림)
INSERT INTO ORDERS VALUES (70,600,'GGG',7); --O (실행됨)
...
INSERT INTO TEST1 VALUES (10,''); --NN 제약조건으로 인해 에러

SELECT * FROM TEST1;
DESC TEST1;
--NO의 NOT NULL은 PK로 인한 NOT NULL (PK로  설정함에 따라 따라오는 제약조건)
--NAME의 NOT NULL은 NN로 인한 NOT NULL
...
SELECT * FROM USER_CONSTRAINTS; --CUSTOMER_ID_PK에 DISABLED라고 적혀있다.

INSERT INTO CUSTOMER VALUES (1,'SHIN',20);
SELECT * FROM CUSTOMER;
--PK를 비활성화하면 중복값이 있어도 데이터가 들어가게 된다.

ALTER TABLE CUSTOMER
ENABLE PRIMARY KEY;
--비활성화를 취소하려고 하면 에러가 발생한다.
--왜냐하면 위에서 데이터를 넣음으로써 PK를 활성화하려고 하는 컬럼에 이미 중복값이 있기 때문이다.
--PK 활성화를 위해 데이터를 지워야 한다.
DELETE CUSTOMER WHERE NAME='SHIN';
INSERT INTO CUSTOMER VALUES (1,'SHIN',20);
--데이터를 지우고 PK를 다시 활성화 한 후에 위와 같은 데이터를 다시 입력하려고 하면 에러가 발생한다.
--PK 제약조건에 걸리기 때문
...
--ORDERS의 PK를 참조하고 있는 CATALOG의 FK도 비활성화 된다.
...
--DBA가 KIM한테 CREATE할 권한을 주지 않아서 실행 오류
--SYS에서 GRANT를 실행하고 다시 위 문장을 재실행하면 정상적으로 실행된다.

SELECT * FROM PER10_V;
--사실 SELECT * FROM PERSONNEL WHERE DNO=10; 이 실행되는 것이다.
...
--JOIN문으로 만든 VIEW는 DML(INSERT,UPDATE,DELETE)이 안된다.
...
--PERSONNEL 테이블에는 PNO가 있다. 그런데 만든 뷰에는 PNO가 없다.
--PNO에는 널값이 들어갈 수 없기 때문에 INSERT를 할 때 에러가 발생한다.

SELECT * FROM PER_AVG;

INSERT INTO PER_AVG VALUES (40,1234,5000); --X
--PER_AVG 테이블은 기존의 데이터(PERSONNEL테이블)를 정리해서 만든 테이블이다.
--따라서 이 테이블(PER_AVG)에 데이터를 추가하고 싶으면 원 데이터가 있는 테이블(PERSONNEL)에 데이터를 넣어주어야 한다.


--VIEW 수정
--CREATE로 만들게 되면 ATLER로 수정한다. 하지만 VIEW는 CREATE OR REPLACE로 수정한다.
...
--JOIN문으로 VIEW를 만들면 DML(INSERT, UPDATE, DELETE)이 안된다.
CREATE TABLE 고객정보
(고객번호 CHAR(10),
이름 CHAR(10));
...
INSERT INTO 정보 VALUES ('A003','정인선','SAMSUNG'); --X
UPDATE 정보 SET 이름='정인선' WHERE 고객번호='A001'; --X
...
--JOIN문으로 만든 VIEW가 아니어도 값을 수정하지 못하게 조건을 건다.
--CHECK 제한을 준 컬럼에만 제약조건을 준다. 제한이 없는 다른 데이터는 수정 가능하다.
...
--WITH READ ONLY
--DML 전부를 제한한다. 읽기모드. SELECT만 가능하다.
CREATE OR REPLACE VIEW PER10_V
AS
...
--ROWNUM: 데이터가 몇개건 상관없이 셀렉트 하는 그 순간의 일련번호
...
ORDER BY STARTDATE DESC)
WHERE ROWNUM<=5;
--정렬이 먼저 된 후에 ROWNUM으로 인해 일련번호가 붙는다.
...
--MS-SQL 방법
SELECT TOP 5 PNAME,STARTDATE FROM PERSONNEL ORDER BY STARTDATE DESC; --상위 5명
SELECT TOP 5 PERCENT PNAME,STARTDATE FROM PERSONNEL ORDER BY STARTDATE DESC; --상위 5%
SELECT TOP 5 PNAME,STARTDATE FROM PERSONNEL ORDER BY STARTDATE ASC; --하위 5명
...
SELECT * FROM PERSONNEL WHERE DNO=10;
--인덱스는 WHERE절 뒤에 나오는 컬럼에 작성하는 것이다.


DROP INDEX PER_PAY_IDX;
--인덱스를 지우기 위해선 DROP을 사용한다.
--PK가 지워지면 인덱스도 지워진다.



--SEQUENCE (일련번호)
--한번 쓰면 다시 사용할 수 없다. 중복되지 않는 값을 자동으로 넣어주기 위해 사용한다. 숫자만 들어간다.
CREATE SEQUENCE DIV_DNO
START WITH 1 --1부터 시가
INCREMENT BY 1 --1씩 증가
MAXVALUE 10 --10번까지 넣을 것이다. / NOMAXVALUE
NOCYCLE --CYCLE
CACHE 20; --CACHE
...
--인서트하면 들어가는 번호. 입력하는 명령어

--CURRVAL : SEQUENCE번호.CURRTVAL
--마지막으로 사용하는 번호. 방금 집어넣은 데이터를 확인하는 명령어
